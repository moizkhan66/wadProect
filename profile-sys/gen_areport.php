<?php
    ...
    if(isset($_GET['a_id']) && isset($_GET['stud_num']))
    {
        ...
        $sql = 'SELECT `anecdotal_date`, `anecdotal_note`, `anecdotal_giver`, 
            `giver_rltn`, `user_id`, `adate_encoded`, `studstatus_id`, `term` FROM `anecdotal_tb` WHERE anecdotal_id = '.$aId;
    ...
    class myPDF extends FPDF{
        ...
        function __construct($studNum, $studLname,$studFname,$gradeLvl,$section,$bDate, $gender, $nationality, $ayName, $term, $aDate, $aNote, $aGiver, $giverRltn, $aDateEncode, $encodeBy) {
            parent::__construct();
            ...
            $this->aNote = $aNote;
            $this->aGiver = $aGiver;
            $this->giverRltn = $giverRltn;
            $this->aDateEncode = $aDateEncode;
        ...
        function headerTable(){
            ...
            $this->MultiAlignCell(35,10,'ANECDOTAL DATE',1,0,'C');
            $this->MultiAlignCell(35,10,'ANECDOTAL Note',1,0,'C');
            $this->MultiAlignCell(35,10,'ANECDOTAL GIVER',1,0,'C');
            $this->MultiAlignCell(35,20,'RELATIONSHIP TO STUDENT',1,0,'C');
            ...
            $this->MultiAlignCell(35,20,$this->aDate,1,0,'C');
            $this->MultiAlignCell(35,20,$this->aNote,1,0,'C');
            $this->MultiAlignCell(35,20,$this->aGiver,1,0,'C');
            $this->MultiAlignCell(35,20,$this->giverRltn,1,0,'C');
            $this->MultiAlignCell(35,10,$this->aDateEncode,1,0,'C');
    ...
    $pdf = new myPDF($studNum, $student['stud_lname'], $student['stud_fname'], $gradeLvl, $section, $student['birthdate'], 
        $student['gender'], $student['nationality'], $ayName, $row['term'], $row['anecdotal_date'], $row['anecdotal_note'],
        $row['anecdotal_giver'], $row['giver_rltn'], $row['adate_encoded'], $user['user_username']);
    $pdf->AliasNbPages();
    $pdf->AddPage('L','A4',0);
    $pdf->headerTable();
    $pdf->Output();
